...
class GudangBahanBakuController extends Controller
{
    ...
    {
        $data = BarangDatangDetail::groupBy('purchaseId')
    ...
    {
        $dtPurchase = [];
        $purchaseIds = AdminPurchase::where('jenisPurchase', 'Purchase Order')
            ->groupBy('kode')
            ->get();
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($barangDatang->save()) {
                ...
                DB::commit();
                return redirect('/bahan_baku/supply')->with('success', 'Data Berhasil Ditambahkan');
            } else {
                return redirect()
                    ->back()
                    ->with('error', 'Data Gagal Ditambahkan, Lengkapi Data Yang Dibutuhkan');
            }
        } catch (\Throwable $th) {
            DB::rollback();
    ...
    public function detail($id)
    {
        ...
        foreach ($barangDatang as $key => $value) {
            ...
            foreach ($value->detail as $key => $value2) {
                $value2->detailMaterial = BarangDatangDetailMaterial::select('*', DB::raw('count(id) as jumlahRoll'))
                    ->where('barangDatangDetailId', $value2->id)
                    ->groupBy('diameter', 'gramasi', 'brutto', 'netto', 'tarra')
                    ->get();
    ...
    public function update($id, Request $request)
    {
        ...
        try {
            ...
            $dataBahanBaku['qtySaatIni'] = array_sum($request['qtySaatIni']);
            GudangBahanBakuDetail::where('datangDetailId', $request['detailId'][0])->update($dataBahanBaku);
            $bahanBakuDetailId = GudangBahanBakuDetail::where('datangDetailId', $request['detailId'][0])->first();

            GudangBahanBakuDetailMaterial::where('gudangDetailId', $bahanBakuDetailId->id)->update([
                'gramasi' => array_sum($request['gramasi']),
                'diameter' => array_sum($request['diameter']),
                'brutto' => array_sum($request['brutto']),
                'netto' => array_sum($request['netto']),
                'tarra' => array_sum($request['tarra']),
                'userId' => \Auth::user()->id,
            ]);

            DB::commit();
            return redirect('bahan_baku/supply')->with('success', 'Data Berhasil Diubah');
        } catch (\Throwable $th) {
            DB::rollback();
            if (env('APP_ENV') == 'local') {
                throw $th;
    ...
    {
        $data = [];
        $data[0] = GudangRajutKeluarDetail::groupBy('gdRajutKId')
            ->whereHas('material', function (Builder $query) {
                $query->where('keterangan', 'LIKE', '%Bahan Baku%');
            })
            ->get();
        $data[1] = GudangCuciKeluarDetail::groupBy('gdCuciKId')
            ->whereHas('material', function (Builder $query) {
                $query->where('keterangan', 'LIKE', '%Bahan Baku%');
            })
            ->get();
        $data[2] = GudangCompactKeluarDetail::groupBy('gdCompactKId')
            ->whereHas('material', function (Builder $query) {
                $query->where('keterangan', 'LIKE', '%Bahan Baku%');
            })
            ->get();
        $data[3] = GudangInspeksiKeluarDetail::groupBy('gdInspeksiKId')
            ->whereHas('material', function (Builder $query) {
                $query->where('keterangan', 'LIKE', '%Bahan Baku%');
            })
            ->get();
        $data[4] = GudangPotongKeluarDetail::groupBy('gdPotongKId')
            ->whereHas('material', function (Builder $query) {
                $query->where('keterangan', 'LIKE', '%Bahan Baku%');
            })
            ->get();
            ...
            foreach ($data[$i] as $val) {
                switch ($i) {
                    ...
                    case 2:
                        ...
                        $val->foreign = $val->compact->cuci->cuciDetail->id;
    ...
    public function getDataMaterial($gudangRequest, $jenisKain = null)
    {
        if ($gudangRequest != 4 || ($gudangRequest == 4 && $jenisKain == 1)) {
            ...
            $datas = GudangBahanBakuDetail::where('materialId', $data['material']->id)
                ->groupby('purchaseId')
                ->get();
            $data['purchase'] = [];
            ...
            $datas = GudangInspeksiStokOpname::where('materialId', $data['material']->id)
                ->groupby('purchaseId')
                ->get();
            $data['purchase'] = [];
            foreach ($datas as $val) {
    ...
    {
        $datas['diameter'] = [];
        $detailGudang = GudangBahanBakuDetail::where('materialId', $materialId)
            ->where('purchaseId', $purchaseId)
            ->first();
        $gudangMaterialDetail = GudangBahanBakuDetailMaterial::where('gudangDetailId', $detailGudang->id)
            ->where('qty', '!=', 0)
            ->get();
    ...
    {
        $datas['diameter'] = [];
        $detailGudangInspeksi = gudangInspeksiStokOpname::where('materialId', $materialId)
            ->where('purchaseId', $purchaseId)
            ->where('qty', '!=', 0)
            ->get();
    ...
    public function getDataDetailMaterialInspeksi($materialId, $purchaseId, $diameter, $gramasi = '', $berat = '')
    {
        $datas = [];
        if ($gramasi == 'null') {
            $detailGudang = gudangInspeksiStokOpname::where('materialId', $materialId)
                ->where('purchaseId', $purchaseId)
                ->where('diameter', $diameter)
                ->where('qty', '!=', 0)
                ->get();
            ...
            $detailGudang = gudangInspeksiStokOpname::where('materialId', $materialId)
                ...
                ->where('qty', '!=', 0)
                ->first();
            $gudangMaterialDetail = gudangInspeksiStokOpnameDetail::where('gdInspeksiStokId', $detailGudang->id)
                ->where('berat', $berat)
                ->first();
    ...
    public function storeKeluarGudang(Request $request)
    {
        ...
        try {
            ...
            return redirect('/bahan_baku/keluar')->with('success', 'Data Berhasil Disimpan');
        } catch (\Throwable $th) {
            DB::rollback();
            if (env('APP_ENV') == 'local') {
                throw $th;
        ...
        try {
            if ($request->jumlah_data != 0) {
                ...
                if ($keluarDetail == 1) {
                    ...
                    return redirect('/bahan_baku/keluar')->with('success', 'Data Berhasil Diubah');
                }
            } else {
                return redirect('/bahan_baku/keluar')->with('error', 'Tidak Ada Perubahan Untuk Disimpan');
            }
        } catch (\Throwable $th) {
            DB::rollback();
    ...
    public function deleteKeluarGudang(Request $request)
    {
        ...
        try {
            ...
            return redirect('bahan_baku/keluar')->with('success', 'Data Berhasil Dihapus');
        } catch (\Throwable $th) {
            DB::rollback();
            if (env('APP_ENV') == 'local') {
                throw $th;
            ...
            foreach ($data[$i] as $val) {
                switch ($i) {
                    case 0:
                        ...
                        $val->foreign = $val->gdRajutKId;
                        break;

                    case 1:
                        $val->gudangRequest = 'Gudang Compact Masuk';
                        $val->foreign = $val->gdCompactKId;
                        break;

                    case 2:
                        $val->gudangRequest = 'Gudang Inspeksi Masuk';
                        $val->foreign = $val->gdInspeksiKId;
                        break;
                }
            }
        }
    ...
    public function terimaMasukGudang($id, $gudangRequest)
    {
        ...
        try {
            ...
            if ($statusDiterima == 1) {
                ...
                return redirect('bahan_baku/masuk')->with('success', 'Barang Sudah Diterima');
            }
        } catch (\Throwable $th) {
            DB::rollback();
            ...
            if (env('APP_ENV') == 'local') {
                ...
                return redirect()
                    ->back()
                    ->with('error', 'Gagal Terima Barang');
            }
        }
    }
}
