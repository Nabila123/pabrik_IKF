...
class GudangInspeksiController extends Controller
{
    ...
    public function gudangInspeksiRequest()
    {
        ...
        foreach ($gInspeksiRequest as $request) {
            ...
            foreach ($cekDetail as $detail) {
                $gudangInspeksi = gudangInspeksiStokOpname::where('gdInspeksiKId', $detail->gdInspeksiKId)
                    ->where('gdDetailMaterialId', $detail->gdDetailMaterialId)
                    ->where('purchaseId', $detail->purchaseId)
                    ->first();
                $cekBahanPembantu = GudangInspeksiKeluarDetail::where('gdInspeksiKId', $request->id)
    ...
    public function RDetail($id)
    {
        ...
        $cekBahanPembantu = GudangInspeksiKeluarDetail::where('gdInspeksiKId', $id)
            ->whereHas('material', function (Builder $query) {
                $query->where('keterangan', 'LIKE', '%Bahan Bantu / Merchandise%');
            })
            ->get();
    ...
    public function PCreate()
    {
        ...
        foreach ($gudangKeluar as $value) {
            $gudangKeluarDetail = GudangInspeksiKeluarDetail::select('purchaseId', 'gdInspeksiKId', 'gdDetailMaterialId', 'materialId', 'jenisId')
                ->where('gdInspeksiKId', $value->id)
                ->get();
            foreach ($gudangKeluarDetail as $detail) {
                $inspeksiStokOpname = gudangInspeksiStokOpname::where('gdInspeksiKId', $detail->gdInspeksiKId)
                    ->where('gdDetailMaterialId', $detail->gdDetailMaterialId)
                    ->where('purchaseId', $detail->purchaseId)
                    ->first();
    ...
    public function getDataDetailMaterial($purchaseId, $materialId, $diameter, $gramasi = '')
    {
        ...
        $datas = [];
        if ($gramasi == 'null') {
            $inspeksiKeluarDetail = GudangInspeksiKeluarDetail::where('gdInspeksiKId', $dataId[1])
