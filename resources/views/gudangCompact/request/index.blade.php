                                 ...
                                 <tbody class="textAlign">
                                     @foreach ($gCompactRequest as $detail)
                                         <tr>
                                             ...
                                             <td>
                                                 @if ($detail->statusDiterima == 0)
                                                     @if (\Auth::user()->roleId == 1 ||
                                                         ...
                                                         \Auth::user()->roleId == 7 ||
                                                         \Auth::user()->roleId == 15)
                                                         <a href="{{ route('GCompact.request.terima', [$detail->id]) }}"
                                                     ...
                                                     @if ($detail->statusDiterima == 1 && !isset($detail->cekPengembalian))
                                                         @if (\Auth::user()->roleId == 1 ||
                                                             ...
                                                             <input type="hidden" name="gudangKeluarId" id="gudangKeluarId"
                                                                 value="{{ $detail->id }}">
                                                             <a href="{{ route('GCompact.request.kembali', [$detail->id]) }}"
                                                                 class="btn btn-info requestKode" style="font-size: 13px;">
                                                                 Ajukan Pengembalian </a> <br>
                                                             <span style="color: green; font-size: 10px">Kembalikan Barang
                                                             </span>
                                                         @else
                                             ...
                                             <td>
                                                 <a href="{{ route('GCompact.request.detail', [$detail->id]) }}"
