                     ...
                     <div class="card">
                         <div class="card-header">
                             @if (\Auth::user()->roleId == 1 ||
                                 ...
                                 \Auth::user()->roleId == 9)
                                 <h3 class="card-title">
                                     <a href="{{ route('adminPO.poRequest.create') }}"
                             ...
                             <table id="example2" class="table table-bordered dataTables_scrollBody" style="width: 100%;">
                                 <thead>
                                     <tr>
                                         ...
                                         <th rowspan="2" class="textAlign" style="vertical-align: middle;">Tanggal
                                             Pengajuan</th>
                                         <th colspan="3" class="textAlign" style="vertical-align: middle;">Agreement
                                         </th>
                                         <th colspan="2" class="textAlign" style="vertical-align: middle;">Status</th>
                                         <th rowspan="2" class="textAlign" style="vertical-align: middle; width:11%;">
                                             action</th>
                                     </tr>
                                     <tr>
                                         ...
                                         <tr>
                                             <td>
                                                 @if (\Auth::user()->roleId == 6 && $request->isKaDeptPO != 0 && $request->prosesOrder != true)
                                                     <a href="{{ route('adminPO.poRequest.requestKode', $request->id) }}"
                                                         class="btn btn-info requestKode"> Buat Pruchase Order </a> <br>
                                                     <span style="color: green; font-size: 10px">Lanjut Purchase
                                                         Order</span>
                                                 @else
                                                     {{ $request->kode }}
                                                 @endif
                                             ...
                                             <td>
                                                 ...
                                                 <span style="color: green; font-size: 10px">Submitted At <br>
                                                     {{ date('d F Y', strtotime($request->tanggal)) }}</span>

                                             </td>
                                             <td>
                                                 @if ($request->isKaDeptProd != 0)
                                                     {{ $request->roleKaDeptProdUser->nama }} <br>
                                                     <span style="color: green; font-size: 10px"> Aproved At <br>
                                                         {{ date('d F Y', strtotime($request->isKaDeptProdAt)) }} </span>
                                                 @else
                                                     @if (\Auth::user()->roleId == 7)
                                                         <button type="button" id="approveKaDeptProd"
                                                             purchaseid="{{ $request->id }}"
                                                             class='btn btn-success approveKaDeptProd'><i
                                                                 class="fas fa-check-square" style="font-size: 14px"> </i>
                                                             Approve</a>
                                                         @else
                                                             <br>
                                                             <span style="color: rgb(253, 5, 5); font-size: 10px">Dalam
                                                                 Proses Approve</span>
                                                     @endif
                                                 @endif
                                             </td>
                                             <td>
                                                 @if ($request->isKaDeptPO != 0)
                                                     ...
                                                     <span style="color: green; font-size: 10px"> Aproved At <br>
                                                         {{ date('d F Y', strtotime($request->isKaDeptPOAt)) }} </span>
                                                 @else
                                                     @if (\Auth::user()->roleId == 6 && $request->isKaDeptProd != 0)
                                                         <button type="button" id="approveKaDeptPO"
                                                             purchaseId="{{ $request->id }}"
                                                             class='btn btn-success approveKaDeptPO'><i
                                                                 class="fas fa-check-square" style="font-size: 14px"> </i>
                                                             Approve</a>
                                                         @else
                                                             <br>
                                             ...
                                             <td>
                                                 @if ($request->prosesOrder == true)
                                                     <span style="color: green; font-size: 10px">
                                                         {{ date('d F Y', strtotime($request->prosesOrderAt)) }} </span>
                                                 @else
                                             ...
                                             <td>
                                                 @if ($request->barangDatang == true)
                                                     ...
                                                     <span style="color: green; font-size: 10px"> At
                                                         {{ date('d F Y', strtotime($request->barangDatangAt)) }} </span>
                                                 @else
                                             ...
                                             <td>
                                                 <a href="{{ route('adminPO.poRequest.detail', $request->id) }}"
                                                 ...
                                                 @if (\Auth::user()->roleId == 1 ||
                                                     ...
                                                     \Auth::user()->roleId == 9)
                                                     @if ($request->prosesOrder != true)
                                                         <a href="{{ route('adminPO.poRequest.update', $request->id) }}"
                                                             class='btn btn-success'><i class="fas fa-pencil-alt"
                                                                 style="font-size: 14px"></i></a>
                                                     @else
                                                         <button type="button" class='btn btn-success disabled mt-1'><i
                                                 ...
                                                 @if ($request->isKaDeptPO != 0)
                                                     <a href="{{ route('adminPO.poRequest.unduh', $request->id) }}"
                                                         target="_blank" class='btn btn-info mt-1'><i
                                                             class="fas fa-download" style="font-size: 14px"></i></a>
                                                 @else
                                                     <button type="button" class='btn btn-info disabled mt-1'><i
                                                             class="fas fa-download" style="font-size: 14px"></i></button>
                                                 @endif
                                                 ...
                                                 @if (\Auth::user()->roleId == 1 ||
                                                     ...
                                                     <button type="button" data-toggle="modal"
                                                         purchaseId='{{ $request->id }}' data-target="#DeleteModal"
                                                         id="modalDelete" onclick='deleteData("{{ $request->id }}")'
